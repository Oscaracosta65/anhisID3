 ...
 * - $gName
 * - $dbCol
 * - $gId     (ID3 = Idaho Weekly Grand, CO2 = Colorado Cash 5)
 *
 * GAME CONFIG:
 * - Pick 5 game (main balls: first, second, third, fourth, fifth)
 * - Main number range: 01..32
 * - No separate bonus ball pool
 */
...
$metaDescBase = (isset($stateName, $gName) && $stateName !== '' && $gName !== '')
    ? 'Analyze ' . $stateName . ' ' . $gName . ' lottery results. Frequency, recency, hot and quiet numbers, draw history, and advanced AI analysis tools at LottoExpert.net.'
    : 'Lottery results analysis — frequency, recency, hot and cold numbers, draw history, and advanced AI tools at LottoExpert.net.';
$doc->setMetaData('description', $metaDescBase);
 ...
 * --------------------------------------------------------------------------
 */
$mainBallCols  = ['first', 'second', 'third', 'fourth', 'fifth'];
$mainBallCount = 5;
$mainMinRange  = 1;
$mainMaxRange  = 32;
$hasBonusBall  = false;

$gameIdSafe = (isset($gId) && $gId !== '') ? (string) $gId : 'ID3';
...
/**
 * --------------------------------------------------------------------------
 * Game-specific links (per game ID)
 * --------------------------------------------------------------------------
 */
$gameLinks = [
    'ID3' => [
        'skai'         => '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=ID3',
        'ai'           => '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=ID3',
        'heatmap'      => '/all-lottery-heatmaps?gameId=ID3',
        'skip'         => '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=ID3',
        'mcmc'         => '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=ID3',
        'heatmapLabel' => 'Weekly Grand Heatmap',
    ],
    'CO2' => [
        'skai'         => '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=CO2',
        'ai'           => '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=CO2',
        'heatmap'      => '/all-lottery-heatmaps?gameId=CO2',
        'skip'         => '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=CO2',
        'mcmc'         => '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=CO2',
        'heatmapLabel' => 'Cash 5 Heatmap',
    ],
];

$currentGameLinks = isset($gameLinks[$gameIdSafe]) ? $gameLinks[$gameIdSafe] : [
    'skai'         => '#',
    'ai'           => '#',
    'heatmap'      => '#',
    'skip'         => '#',
    'mcmc'         => '#',
    'heatmapLabel' => 'Heatmap Analysis',
];
 ...
 * --------------------------------------------------------------------------
 */
$latestRows = leFetchRecentDraws($db, (string) $dbCol, $gameIdSafe, 1);
...
$rowsMain = leFetchRecentDraws($db, (string) $dbCol, $gameIdSafe, $nodCurrentMain);

[$mainCounts, $mainLastSeenIndex] = leInitRange($mainMinRange, $mainMaxRange);
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels   = leBuildNaturalLabels($mainMinRange, $mainMaxRange);
$mainChartValues   = [];
$mainRecencyValues = [];
...
$window50  = leFetchRecentDraws($db, (string) $dbCol, $gameIdSafe, 50);
$window300 = leFetchRecentDraws($db, (string) $dbCol, $gameIdSafe, 300);

[$counts50, ]  = leInitRange($mainMinRange, $mainMaxRange);
[$counts300, ] = leInitRange($mainMinRange, $mainMaxRange);
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $gameIdSafe, $drawDate, $ball, $mainBallCols);
        $drawsAgo = leGetDrawingsSinceDate($db, (string) $dbCol, $gameIdSafe, $prevDate, $drawDate);
        $drawHistoryRows[] = [
            'label'    => lePad2($ball),
            'prevDate' => $prevDate,
            'drawsAgo' => $drawsAgo,
...
/**
 * --------------------------------------------------------------------------
 * Archive and utility routes
 * --------------------------------------------------------------------------
 */
$archiveRoute  = '/lottery-archives-pick5';
$lowestRoute   = '/lowest-drawn-number-analysis';
$archiveParams = '?gId='       . rawurlencode($gameIdSafe)
...
$mainRangeLabel = '01&ndash;' . str_pad((string) $mainMaxRange, 2, '0', STR_PAD_LEFT);
...
$jsonLdName = (isset($stateName, $gName) && $stateName !== '' && $gName !== '')
    ? $stateName . ' ' . $gName . ' Results Analysis'
    : 'Lottery Results Analysis';
$jsonLdDesc = (isset($stateName, $gName) && $stateName !== '' && $gName !== '')
    ? 'Frequency, recency, and draw history analysis for ' . $stateName . ' ' . $gName . ' lottery.'
    : 'Lottery draw frequency and recency analysis.';
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="<?php echo htmlspecialchars($currentGameLinks['skai'], ENT_QUOTES, 'UTF-8'); ?>">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="<?php echo htmlspecialchars($currentGameLinks['ai'], ENT_QUOTES, 'UTF-8'); ?>">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($currentGameLinks['skip'], ENT_QUOTES, 'UTF-8'); ?>">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($currentGameLinks['mcmc'], ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($currentGameLinks['heatmap'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($currentGameLinks['heatmapLabel'], ENT_QUOTES, 'UTF-8'); ?></a>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel shows the complete main-number distribution across all values <?php echo $mainRangeLabel; ?>. The second panel shows how far each number sits from its most recent draw, allowing you to identify long quiet stretches at a glance.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values <?php echo $mainRangeLabel; ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for values <?php echo $mainRangeLabel; ?></span>
        </div>
        ...
        <div class="skai-table-wrap">
          <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
            ...
            <tbody>
              <?php for ($i = $mainMinRange; $i <= $mainMaxRange; $i++) : ?>
                <?php
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($currentGameLinks['skai'], ENT_QUOTES, 'UTF-8'); ?>">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($currentGameLinks['ai'], ENT_QUOTES, 'UTF-8'); ?>">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($currentGameLinks['skip'], ENT_QUOTES, 'UTF-8'); ?>">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($currentGameLinks['mcmc'], ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($currentGameLinks['heatmap'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($currentGameLinks['heatmapLabel'], ENT_QUOTES, 'UTF-8'); ?></a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($archiveRoute . $archiveParams, ENT_QUOTES, 'UTF-8'); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($lowestRoute . $archiveParams, ENT_QUOTES, 'UTF-8'); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
